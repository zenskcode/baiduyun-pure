...
<head>
	...
	<link rel="stylesheet" enabled id="ColorMode-Light" href="https://cdn.staticfile.org/twitter-bootstrap/4.1.2/css/bootstrap.min.css" />
	<link rel="stylesheet" enabled id="Swal2-Light" href="https://cdn.jsdelivr.net/npm/@sweetalert2/theme-default@4.0.2/default.min.css" />
...
<body>
	<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
		<div class="container">
			<a class="navbar-brand" href="./"><img src="static/logo.png" class="img-fluid rounded logo-img mr-2" alt="LOGO" />百度网盘直链解析</a>
				...
				if (isset($_POST["fs_id"]) && isset($_POST["time"]) && isset($_POST["sign"]) && isset($_POST["randsk"]) && isset($_POST["share_id"]) && isset($_POST["uk"]) && isset($_POST["bdstoken"]) && isset($_POST["filesize"])) {
					...
					if ($json4["errno"] == 0) {
						...
						if (USING_DB and $result = mysqli_fetch_assoc($mysql_query)) {
							...
							else $headerArray = array('User-Agent: netdisk', 'Cookie: BDUSS=' . $SVIP_BDUSS . ';'); // 仅此处用到SVIPBDUSS
							...
							<div class="row justify-content-center">
								<div class="col-md-7 col-sm-8 col-11">
									<div class="alert alert-primary" role="alert">
								        ...
								        <hr />
								<p class="card-text">
									<a id="https" href="https://' . $realLink . '" target="_blank" rel="nofollow noopener noreferrer">' . Language["DownloadLink"] . '（' . ($smallfile ? '无需' : '需要') . '设置 UA，' . DownloadLinkAvailableTime 
									    . '小时内有效）</a></p>';
										?>
				...
				<div class="card">
					...
					<div class="card-body">
						<form name="form1" method="post" onsubmit="return validateForm()">
							<div class="form-group my-2"><input type="text" autocomplete="off" class="form-control" name="surl" placeholder="<?php echo Language["ShareLink"]; ?>" oninput="Getpw()"></div>
							<div class="form-group my-4"><input type="text" autocomplete="off" class="form-control" name="pwd" placeholder="<?php echo Language["SharePassword"]; ?>"></div>
							<?php
							if (IsCheckPassword) {
